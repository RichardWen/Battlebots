...
void SpriteSheet::Draw(float xLocation, float yLocation) {
	gfx->GetRenderTarget()->DrawBitmap(bmp, D2D1::RectF(xLocation, yLocation, xLocation + bmp->GetSize().width, yLocation + bmp->GetSize().height), 1.0f, D2D1_BITMAP_INTERPOLATION_MODE::D2D1_BITMAP_INTERPOLATION_MODE_NEAREST_NEIGHBOR, D2D1::RectF(0.0f, 0.0f, bmp->GetSize().width, bmp->GetSize().height));
}
